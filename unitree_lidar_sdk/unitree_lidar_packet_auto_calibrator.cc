...
DEFINE_string(input_path, "/tmp/unitree_lidar_packets.bin",
              "Input raw packet recording file.");
DEFINE_int32(accumulate_rings, 50,
             "Number of packets to accumulate into one displayed cloud.");
...
DEFINE_double(merged_point_size, 1.0,
              "Point size for the merged initial-frame cloud.");
...
DEFINE_int32(merge_beginning_frames, 1000,
             ...
             "overlay it.");
DEFINE_bool(orthographic_camera, true,
            "Use an orthographic camera in the Pangolin viewer.");
DEFINE_double(
    orthographic_extent, 10.0,
    "Half extent of the orthographic camera frustum in viewer units.");

DEFINE_bool(extract_planes, true,
            "Extract planes from the merged beginning cloud.");
DEFINE_int32(max_planes, 3,
             "Maximum number of planes to extract from the merged cloud.");
DEFINE_double(plane_inlier_threshold_m, 0.2,
              "Point-to-plane distance threshold used for plane extraction and "
              "assignment.");
DEFINE_int32(plane_ransac_iterations, 600,
             "RANSAC iterations per extracted plane.");
DEFINE_int32(plane_min_inliers, 400,
             "Minimum inlier count required to accept an extracted plane.");
DEFINE_int32(plane_detection_sample_limit, 25000,
             "Maximum number of merged points used for plane detection.");
DEFINE_double(plane_min_extent_m, 0.75,
              "Minimum plane span along each in-plane axis.");

DEFINE_bool(show_planes, true, "Overlay extracted plane rectangles.");
DEFINE_bool(show_plane_inliers, true,
            "Overlay merged-cloud plane inlier points.");

DEFINE_bool(optimize_calibration, true,
            "Optimize calibration against extracted planes.");
DEFINE_string(
    range_model_candidates, "quadratic",
    "Comma-separated range correction models: constant, linear, quadratic.");
DEFINE_int32(calibration_iterations, 6,
             "Coordinate-descent iterations per model family.");
DEFINE_double(calibration_range_step_m, 0.01,
              "Initial step size for range correction coefficients.");
DEFINE_bool(
    optimize_range_coefficients, true,
    "Optimize delta_range_alpha_fcn coefficients during calibration search.");
DEFINE_double(
    calibration_alpha_step_rad, 0.0005,
...
DEFINE_double(
    calibration_assignment_threshold_m, 0.08,
...
calibration::CalibrationOptimizationConfig
MakeCalibrationOptimizationConfig(int min_required_assigned_points) {
  ...
  config.optimize_alpha_theta_coefficients =
      FLAGS_optimize_alpha_theta_coefficients;
...
calibration::ViewerConfig MakeViewerConfig() {
  ...
  config.initial_calibration_parameters.range_coefficients_m = {0.0f, 0.0f,
                                                                0.0f};
  config.initial_calibration_parameters.alpha_theta_coefficients_rad = {0.0f,
                                                                        0.0f};
  return config;
}
...
int Run(int argc, char **argv) {
  ...
  LOG(INFO) << "Loaded " << packets.size() << " raw lidar packets from "
            << FLAGS_input_path;
  ...
  LOG(INFO) << "Prepared " << frames.size()
            << " replay frames using accumulate_rings="
  ...
  const calibration::PlaneExtractionConfig plane_config =
      MakePlaneExtractionConfig();
  ...
  if (FLAGS_merge_beginning_frames > 0) {
    merged_beginning_frame = std::make_unique<calibration::ReplayFrame>(
        calibration::BuildMergedBeginningFrame(frames,
                                               FLAGS_merge_beginning_frames));
    calibration::RebuildFramePoints(active_calibration,
                                    merged_beginning_frame.get());
    ...
    if (!planes.empty()) {
      const calibration::ResidualSummary baseline =
          calibration::EvaluateResiduals(
              *merged_beginning_frame, planes,
              calibration_config.assignment_threshold_m);
      LOG(INFO) << "Baseline residuals: assigned=" << baseline.assigned_points
                << "/" << baseline.total_points
                << " mean_abs=" << baseline.mean_abs_error_m
                << " rms=" << baseline.rms_error_m
                << " max=" << baseline.max_abs_error_m;

      if (calibration_config.enabled) {
        const calibration::CalibrationSolution solution =
            calibration::OptimizeCalibration(*merged_beginning_frame, planes,
                                             calibration_config);
        if (std::isfinite(solution.objective)) {
          LOG(INFO) << "Selected calibration model=" << solution.model_name
                    << " coeff_m="
                    << calibration::VectorSummary(
                           solution.parameters.range_coefficients_m)
                    << " alpha_theta_coeff_rad="
                    << calibration::VectorSummary(
                           solution.parameters.alpha_theta_coefficients_rad)
          ...
          LOG(WARNING)
              << "Calibration optimization failed to find a finite solution.";
        }
      }
    } else {
