...
DEFINE_bool(extract_planes, false,
...
DEFINE_double(manual_range_c0, 0.0,
              "Initial manual range calibration coefficient c0.");
DEFINE_double(manual_range_c1, 0.0,
              "Initial manual range calibration coefficient c1.");
DEFINE_double(manual_range_c2, 0.0,
              "Initial manual range calibration coefficient c2.");
DEFINE_double(manual_alpha_t0, 0.0,
              "Initial manual theta-alpha calibration coefficient t0.");
DEFINE_double(manual_alpha_t1, 0.0,
              "Initial manual theta-alpha calibration coefficient t1.");
...
calibration::ViewerConfig
MakeViewerConfig(const calibration::CalibrationParameters &parameters) {
...
calibration::CalibrationParameters MakeManualCalibrationParameters() {
  ...
  parameters.alpha_theta_coefficients_rad = {
      static_cast<float>(FLAGS_manual_alpha_t0),
...
int Run(int argc, char **argv) {
  ...
  const calibration::CalibrationParameters parameters =
      MakeManualCalibrationParameters();
  ...
  if (FLAGS_merge_beginning_frames > 0) {
    ...
    calibration::RebuildFramePoints(calibration_state,
    ...
    if (!planes.empty()) {
      const calibration::ResidualSummary residuals =
          calibration::EvaluateResiduals(*merged_beginning_frame, planes,
                                         FLAGS_plane_inlier_threshold_m);
      LOG(INFO) << "Manual calibration residuals: assigned="
                << residuals.assigned_points << "/" << residuals.total_points
                << " mean_abs=" << residuals.mean_abs_error_m
                << " rms=" << residuals.rms_error_m
                << " max=" << residuals.max_abs_error_m;
    }
  }
